...
class Subscribe( object ):
	...
	def _request( self ):
		...
		run.Run( "wget --post-data '%(postData)s'  http://%(server)s/mailman/subscribe/%(listName)s" % dict( postData = postData, server = self._server, listName = self._listName ) )
	...
	def _confirm( self, cookie ):
		...
		run.Run( "wget --post-data '%(postData)s' http://%(server)s/mailman/confirm/%(listName)s" % dict( server = self._server, postData = postData, listName = self._listName ) )
